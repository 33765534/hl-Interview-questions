...
根据我的经验总结：性能优化原则就是利用空间换时间，多使用内存、缓存或者其他方法，减少CPU计算量，减少网络加载耗时
从加载更快角度考虑可用：
1.减少资源体积：压缩代码，如：
(1)webpack中mode：‘praduction’模式，他是为了生产环境，会开启各种优化，比如压缩代码、混淆变量名等，最终让代码更小、运行更快，但是错误和警告信息会被减少或去除。
...
3.第三方库可以按需引入，可以减少所占空间，不能按需引入的可用采用CDN外部加载(使用cdn加速)
...
然后将 listen 443 ssl;改成 listen 443 ssl http2; 即可。`,
      },
      {
        question: "组件封装",
        answer: `
1.背景：由于客户经常反馈页面操作不流畅的问题，因为项目是angularJS项目前后台没分离的老项目，它每次操作的是真实dom,有些渲染上就比较慢，
加上代码老旧化，很多第三方软件也老旧化不安全，作者也不维护了。
方案：部门决策使用vue3来实现前端页面,项目比较庞大，初步阶段还是使用qiankun来逐步迁移
刚开始我自学vue3框架，快速搭建好项目脚手架，由于刚开始组内成员vue3不是很熟悉，
我会组织培训学习，培训一些常用api可以让大家快速上手
问题：但是开发效率上还是不太乐观，分析重复造轮子的地方过多，比如经典的表格,上面搜索,底下表格筛选出结果以及分页，
这样的一整套的template代码会占到250行左右，每一个column列需要自己去写,，也就是我一进页面就有250行HTML内容，
想要看逻辑或者排查问题相对的也不直观
为了提高开发效率,我封装select组件和table组件库以及组件库文档，template代码量也缩减到20行,开发效率提升20%。

我觉得还能再提升，页面结构相似的，可以做成模板，于是开始研究VScode插件，想让插件根据后台接口的yaml生成我们需要的vue代码,
我们接口文档都是yaml 格式，有URL，入参和出参。初步想法通过ast解析yaml然后生成vue代码。在家捣鼓做了个简易的demo发布之后，
想在公司试验，发现公司不支持使用插件，被屏蔽了，安全性问题。就放弃了这个方案，改用了本地代码片段，来快速生成代码。

封装组件设计：
整体设计：
组件库首先统一的设计风格和视觉风格。还要需考虑组件间关系和依赖，保证其协同工作。还有清晰的层次结构和命名规范将便于查找和使用组件。
比如所有组件统一以bpc开头

代码规范配置：
组件库开发代码规范要统一，如缩进、命名规则和编码风格等。
1.可以利用ESLint等工具维持代码一致性，
2.对接github的workflow，不满足规范的代码无法提交、代码合并需专人review等等。

文档设计：
为方便用户快速获取信息，应提供清晰、详尽的文档，包含使用指南、API文档及示例代码，并采用易理解、易搜索的文档结构。
实际的示例和演示也将助于用户更好地理解与使用组件。
1.可以使用vuepress 来实现把md文档生成静态网页
2.每个组件提供详细的使用说明，以及属性方法和插槽的使用说明

使用方便性：
通过提供简洁易用的API和调用方式降低使用门槛，丰富的配置选项可让用户按需定制。封装常用功能可以减轻用户的开发负担。

自定义能力：
组件库应具备灵活的样式定制选项以满足不同设计需求，支持插件机制以便用户添加或扩展功能。
如：1.使用props参数传递来实现
2.使用slot插槽来实现自定义内容
3.通过events触发事件，让父类监听等等

多语言支持：
考虑到国际化需求，组件库应支持多语言环境，提供多种语言的文档和示例，以及语言切换功能，以便用户根据需要轻松切换。
具体实现：1.使用vue-i18n插件来实现多语言
2.使用Pinia状态管理来存储

工程化设计与
1. **包管理**：利用 npm 等包管理工具对组件进行有效管理，并发布到仓库中。
4. **自动化测试**：编写测试用例，确保组件的功能正确性和稳定性，减少人工测试工作量。`,
      },
      {
        question: "Nuxt3",
        answer: `
Nuxt3 提供了一种直观且可扩展的方式来创建类型安全、高性能和生产级别的全栈Web应用和网站，使用的是Vue.js
在开发中享受到热模块替换的便利，并在生产中获得高性能的应用，其中默认启用了服务器端渲染
